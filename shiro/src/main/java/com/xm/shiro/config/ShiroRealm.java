...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        System.out.println("hasUser:"+hasUser.toString());
        //加密   密钥和数据库的密钥进行对比
        String md5 = new Md5Hash(hasUser.getPswd(), token.getUsername(),2).toString();
        if (hasUser != null) {
...
//        String loginName = (String) super.getAvailablePrincipal(principalCollection);
        ...
        System.out.println("权限认证里的UUser:"+user.toString());
        System.out.println("权限认证里的getRoleStrlist:"+user.getRoleStrlist().toString());
        System.out.println("权限认证里的getPerminsStrlist:"+user.getPerminsStrlist().toString());
//        //到数据库查是否有此对象
...
//        user = userMapper.findByName(loginName);
        if (user != null) {
            ...
            info.addStringPermission("userInfo:add");
